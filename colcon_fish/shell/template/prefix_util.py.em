...
FORMAT_STR_REMOVE_TRAILING_SEPARATOR = r'@(shell_extension.FORMAT_STR_REMOVE_TRAILING_SEPARATOR)'  # noqa: E501
...
# Variables that fish treats as lists (space-separated) rather than
...
def _is_fish_list_var(name):
    """Check if a variable is a fish list variable."""
    return name in _FISH_LIST_VARS
...
def _append_unique_value(name, value):
    ...
    if _is_fish_list_var(name):
        # Fish list variable: use 'contains' for dedup and space-separated
        line = 'contains -- "{value}" ${name}; ' \
            'or set -gx {name} ${name} "{value}"'.format(
                name=name, value=value)
    else:
        # Non-list variable: use colon-separated format like sh
...
def _prepend_unique_value(name, value):
    ...
    if _is_fish_list_var(name):
        # Fish list variable: use 'contains' for dedup and space-separated
        line = 'contains -- "{value}" ${name}; ' \
            'or set -gx {name} "{value}" ${name}'.format(
                name=name, value=value)
    else:
        # Non-list variable: use colon-separated format like sh
...
def _remove_ending_separators():
    # For fish list variables, no separator cleanup is needed since they
    # use fish list syntax (space-separated) instead of colon-separated.
    # For non-list variables, we still need to clean up colons.
    ...
    for name in env_state:
        ...
        if name in os.environ:
            continue
        # skip fish list variables - they don't use colon separators
        if _is_fish_list_var(name):
            continue
        commands += [
